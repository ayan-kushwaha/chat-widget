...
import { X, Tag, Check, Sparkles, Plus } from 'lucide-react';
...
export const LabelModal: React.FC<LabelModalProps> = ({ isOpen, onClose, onApply, onCreate, onDelete, existingTags = [], activeTags = [] }) => {
    const [view, setView] = useState<'LIST' | 'ADD'>('LIST');
    const [tagName, setTagName] = useState('');
    const [selectedColor, setSelectedColor] = useState(COLORS[2]); // Default Success
    const [searchTerm, setSearchTerm] = useState('');

    // 🧹 Deduplicate tags by name (keep only unique tag names)
    const uniqueTags = existingTags.reduce((acc: string[], tag) => {
        const [name] = tag.includes(':') ? tag.split(':') : [tag, 'green'];
    ...
    const handleSave = () => {
        ...
        // 🎯 User Request: "add karne pe aplyy nhi hona chahiye"
    ...
    const handleApplyExisting = (tag: string) => {
        ...
        // 🎯 User Request: "list se select add k amlb list new add akrna... aur aplly nhi hona chaiye"
        // Wait, "sab list se select add k matlab list new add karna"...
        // Okay, handleApplyExisting DOES apply it. The user said creation shouldn't apply but selection should.
        onApply(name, color);
    };
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed   left-1/2 top-1/1 -translate-x-1/2 -translate-y-1/2 w-full max-w-[320px]  bg-zinc-950/95 border border-white/10 rounded-3xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.8)] z-[210] overflow-hidden backdrop-saturate-200"
                    >
                        {/* 🌈 Top Accent Line */}
                        ...
                        <div className="p-3 pb-4 space-y-5">
                            {/* 🏷️ Header */}
                            <div className="flex items-center justify-between">
                                ...
                                <button onClick={onClose} className="p-1.5 text-zinc-600 hover:text-white transition-colors bg-white/5 rounded-lg">
                                    <X size={14} />
                                </button>
                            </div>
                            ...
                            {view === 'LIST' ? (
                                <div className="space-y-3">
                                    ...
                                    <div className="relative group">
                                        ...
                                        />
                                        <Tag size={10} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-700 group-focus-within:text-white/40 transition-colors" />
                                    </div>
                                    ...
                                    <ScrollArea className="h-[220px]  pr-4 ">
                                        <div className="grid grid-cols-1 gap-1.5 pb-4">
                                            {filteredTags.length > 0 ? filteredTags.map((tag, idx) => {
                                                ...
                                                // 🎯 Match by name part (Handles color variations)
                                                ...
                                                <div className="py-8 text-center space-y-2">
                                                    ...
                                                    <button onClick={() => { setTagName(searchTerm); setView('ADD'); }} className="text-emerald-500 font-black text-[9px] uppercase tracking-widest hover:underline">
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            "w-full py-2 rounded-xl border border-white/5 font-black text-[9px] uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 group",
                                ...
                                <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    <div className="space-y-2">
                                        <div className="relative group">
                                            ...
                                            />
                                            <Tag size={12} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-700 group-focus-within:text-white/40 transition-colors" />
                                        </div>
                                    </div>
                                            ...
                                            {COLORS.map((color) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        title={color.label}
                                                        className={cn(
                                                            "aspect-square rounded-xl transition-all duration-300 relative flex flex-col items-center justify-center group pb-6",
                                                            "bg-gradient-to-br",
                                                            color.gradient,
