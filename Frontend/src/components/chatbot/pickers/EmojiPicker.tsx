...
import { EmojiMeta, EmojiMetadata } from '@/assets/emogy/EmojiMeta';
...
// Convert codepoint back to emoji char
...
export const EmojiPicker: React.FC<EmojiPickerProps> = ({ onSelect, onClose }) => {
    ...
    const filteredEmojis = useMemo(() => {
        if (search) {
            ...
            return Object.entries(EmojiMeta).filter(([_, data]) => {
                return data.name.includes(lowerSearch) || data.tags.some(t => t.includes(lowerSearch));
            });
        }
        ...
        return Object.entries(EmojiMeta).filter(([_, data]) => data.category === activeCategory);
    ...
    return (
        <motion.div
            ...
            className="w-[320px] h-[350px] bg-white dark:bg-[#1f1f1f] backdrop-blur-xl border border-black/10 dark:border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
        >
            {/* Header */}
            <div className="p-3 border-b border-black/5 dark:border-white/5 bg-black/5 dark:bg-white/5 space-y-3">
                ...
                {/* Categories */}
                <div className="flex gap-1 overflow-x-auto pb-1 no-scrollbar mask-gradient-right">
                    {CATEGORIES.map(cat => {
                        const Icon = cat.icon;
                    ...
                    <div className="grid grid-cols-6 gap-2 pb-2">
                        {filteredEmojis.map(([code, data]) => (
                            <button
                                ...
                                className="aspect-square flex items-center justify-center hover:bg-black/5 dark:hover:bg-white/5 rounded-xl transition-transform hover:scale-110 active:scale-95"
                                title={data.name}
                            >
                                <LottieEmoji
                                    path={data.path}
                                    style={{ width: 32, height: 32 }}
                                    alt={getEmojiFromCodepoint(code)} // 🛡️ Fallback for missing animations
                                />
                            </button>
                        ))}
                    </div>
