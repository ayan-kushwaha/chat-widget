...
`; function u(s, t, e = 1e3) { s && (s.style.animation = "none", s.offsetWidth, s.style.animation = `${t} ${e}ms ease-in-out`, setTimeout(() => { s.style.animation = "" }, e)) } function k(s, t, e, i = 10) { if (!s || !t) return; let o = t.offsetX || 20, a = t.offsetY || 20; s.style.left = "", s.style.right = "", s.style.top = "", s.style.bottom = "", t.horizontal === "left" ? s.style.left = `${o + e + i}px` : s.style.right = `${o + e + i}px`, t.vertical === "top" ? s.style.top = `${a}px` : s.style.bottom = `${a}px` } var g = class {
        constructor() {
            ...
            this.updateAnimation = () => {
                if (this.rafId = null, !this.element) return;
                let t = this.element.getBoundingClientRect(), e = t.left + t.width / 2, i = t.top + t.height / 2, o = this.mouseX - e, a = this.mouseY - i, r = Math.sqrt(o * o + a * a);

                // ⚡ Cross-Iframe Bridge: Forward mouse data to all embedded frames (Avatar + Chat)
                const frames = document.querySelectorAll('iframe');
                frames.forEach(frame => {
                    if (frame.contentWindow) {
                        frame.contentWindow.postMessage({ type: 'CLUAIZ_EXTERNAL_MOUSE', x: o, y: a, r: r }, '*');
                    }
                });

                let n = this.getScaling(), c = 100 * n.factor, l = 1.2 * n.factor, p = r < t.width / 2 + c, m = r < t.width / 2;

                // Tell the iframe if mouse is in magnetic range (p = true/false)
                frames.forEach(frame => {
                    if (frame.contentWindow) {
                        frame.contentWindow.postMessage({ type: 'CLUAIZ_MAGNET_HOVER', isHovered: p }, '*');
                    }
                });

                if (Array.from(this.element.children).forEach(d => { if (d.classList.contains("cluaiz-notification-magnet") || d.tagName === "DIV" || d.tagName === "svg") if (p) { let h = o / l, y = a / l, b = m ? 1.05 : 1; d.style.transition = "transform 0.4s cubic-bezier(0.25, 1, 0.5, 1)", d.style.transform = `translate3d(${h}px, ${y}px, 0) scale(${b})` } else d.style.transition = "transform 0.6s cubic-bezier(0.34, 1.56, 0.64, 1)", d.style.transform = "translate3d(0, 0, 0) scale(1)" }), this.config?.icon.type === "robot" && this.botElements.body) { let f = Math.min(r / 300, 1), h = o / (r || 1) * f * 8, y = a / (r || 1) * f * 6; this.botElements.face && (this.botElements.face.style.setProperty("--offset-x", `${h * .4}px`), this.botElements.face.style.setProperty("--offset-y", `${y * .4}px`)), this.botElements.features && (this.botElements.features.style.setProperty("--offset-x", `${h * 1.2}px`), this.botElements.features.style.setProperty("--offset-y", `${y * 1.2}px`)); let b = 4, M = o / (r || 1) * f * b, E = a / (r || 1) * f * b; this.element.style.setProperty("--pupil-x", `${M}px`), this.element.style.setProperty("--pupil-y", `${E}px`) }
            };
        ...
        initDragLogic() {
            ...
            const move = e => {
                ...
                if (!this.isDragging) {
                    ...
                    const frames = document.querySelectorAll('iframe');
                    frames.forEach(f => f.contentWindow?.postMessage({ type: 'CLUAIZ_DRAG_STATE', dragging: true }, '*'));
                }
            ...
            const up = e => {
                ...
                // ⚡ Broadcast Drag End
                const frames = document.querySelectorAll('iframe');
                frames.forEach(f => f.contentWindow?.postMessage({ type: 'CLUAIZ_DRAG_STATE', dragging: false }, '*'));
        ...
        initMessageListener() {
            window.addEventListener("message", t => {
                ...
                // If child iframe tells us about typing/reading
                if (t.data.type === "CLUAIZ_READING") {
                    // Forward this back down specifically to the avatar widget (which might be in a different iframe or same depending on architecture)
                    // We broadcast it to all frames just in case
                    const frames = document.querySelectorAll('iframe');
                    frames.forEach(frame => {
                        if (frame.contentWindow) {
                            frame.contentWindow.postMessage({ type: 'CLUAIZ_READING', isReading: t.data.isReading }, '*');
                        }
                    });
                }
            });
