"""
    ...
    MANUAL INDEXER                                              
  Cluaiz Neural OS | services/neural/indexer/manual_indexer.py    
                                                                  
  MongoDB: cluaiz > manualdocuments > page_index                  
  Handles: Manually typed / pasted knowledge docs                 
...
class ManualIndexer:
    """
    Indexes manually typed knowledge into manualdocuments collection.
    """
    ...
    async def index(
        ...
        logger.info(f"  [ManualIndexer] Indexing '{filename}' ({len(chunks)} chunks)")
        ...
        async def process(i, text):
            async with sem:
                ...
                try:
                    ...
                    logger.warning(f" [ManualIndexer] Chunk {i} failed: {e}")
                    return None
        ...
        #  Save to MongoDB (manualdocuments) 
        ...
        try:
            res = await db.manualdocuments.update_one(query, update)
            logger.info(f"    [manualdocuments] matched={res.matched_count} modified={res.modified_count}")

            if res.matched_count == 0:
                res = await db.manualdocuments.update_one({"_id": query["_id"]}, update)
                if res.matched_count > 0:
                    logger.info("    [manualdocuments] ID-only fallback matched")
        except Exception as e:
            logger.error(f" [ManualIndexer] MongoDB save failed: {e}")
        ...
        logger.info(f" [ManualIndexer] Done in {duration_ms}ms | {len(nodes)} nodes")
