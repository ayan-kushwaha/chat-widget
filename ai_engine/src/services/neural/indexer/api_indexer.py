"""
    ...
    API INDEXER                                                  
  Cluaiz Neural OS | services/neural/indexer/api_indexer.py       
                                                                  
  MongoDB: cluaiz > apisources > page_index                       
  Handles: Connected API / integration sources                    
...
class ApiIndexer:
    """
    Indexes API/integration sources into apisources collection.
    """
    ...
    async def index(
        ...
        logger.info(f" [ApiIndexer] Indexing '{filename}' ({len(chunks)} chunks)")
        ...
        async def process(i, text):
            async with sem:
                ...
                try:
                    ...
                    logger.warning(f" [ApiIndexer] Chunk {i} failed: {e}")
                    return None
        ...
        try:
            res = await db.apisources.update_one(query, update)
            logger.info(f"    [apisources] matched={res.matched_count} modified={res.modified_count}")

            if res.matched_count == 0:
                res = await db.apisources.update_one({"_id": query["_id"]}, update)
                if res.matched_count > 0:
                    logger.info("    [apisources] ID-only fallback matched")
        except Exception as e:
            logger.error(f" [ApiIndexer] MongoDB save failed: {e}")
        ...
        logger.info(f" [ApiIndexer] Done in {duration_ms}ms | {len(nodes)} nodes")
